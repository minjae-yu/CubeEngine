...
{
	//float newAngle = { angle_ + rand() % 1 - rand() % 360 };
	particles.push_back(std::move(Particle(engine, position_, size_, speed_, angle_, lifeTime, type, spriteName_, color_)));
}
...
{
	particles.push_back(std::move(Particle(engine, position_, size_, speed_, angle_, lifeTime, type, spriteName_, color_, ParticleEffect::FADEOUT)));
